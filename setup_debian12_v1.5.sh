...
start_new_bash_shell() {
    ( # exec bash
command="echo -e \"\033[1;33mEnter the following command:\033[0m\n\033[1;31mRun 'flutter doctor'\033[0m\n\033[1;31mRun 'flutter doctor --android-licenses'\033[0m\""
 # Display warning message in yellow

        gnome-terminal -- bash -c "$command;cd $HOME; exec bash"

        # You are now in a new interactive Bash shell.
        # You can run commands here.

        # For example, let's execute the 'date' command
        # date
...
install_flutter_and_android_studio() {
    ...
    # terminator -u
    # Run 'flutter doctor' in a Terminator window
    # kitty --hold sh -c 'flutter doctror'
    # kitty --hold sh -c 'flutter doctor --android-licenses'

    # Run 'flutter doctor --android-licenses' in another Terminator window
    # terminator -x flutter doctor --android-licenses
    ...
    # gnome-terminal -- bash -c "flutter doctor; exec bash"
    # gnome-terminal -- sh -c 'echo "flutter doctor; sleep 10" | bash'
    ...
    # gnome-terminal -- bash -c ". ~/.bashrc; read -p 'Press Enter to exit...'"
    # gnome-terminal -- bash -c "flutter doctor; read -p 'Press Enter to exit...'"
    # gnome-terminal -- bash -c "flutter doctor --android-licenses; read -p 'Press Enter to exit...'"
    # gnome-terminal -- sh -c 'echo "flutter doctor ; read -p"| bash'
    # gnome-terminal -- sh -c 'echo "flutter doctor --android-licenses; read -p"| bash'

    echo "Flutter and Android Studio installation completed."
}
install_telegram() {
    # Define the URL of the Telegram Desktop tar.xz file
    TELEGRAM_URL="https://telegram.org/dl/desktop/linux"

    # Define the directory where you want to store Telegram
    ...
    wget -O "$TELEGRAM_DIR/telegram.tar.xz" "$TELEGRAM_URL"

    # Extract the tar.xz file
    echo "Extracting Telegram Desktop..."
    tar -xf "$TELEGRAM_DIR/telegram.tar.xz" -C "$TELEGRAM_DIR"

    # Remove the downloaded tar.xz file
    rm "$TELEGRAM_DIR/telegram.tar.xz"

    # Launch Telegram
    echo "Launching Telegram Desktop..."
    "$TELEGRAM_DIR/Telegram/Telegram"

    # Optional: You can create a desktop shortcut for Telegram here if needed.

    echo "Telegram Desktop has been launched."
}
